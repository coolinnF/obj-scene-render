...
class aabb {
    public:
        ...
        aabb(const vec3& min_c_in, const vec3& max_c_in) { min_cor = min_c_in; max_cor = max_c_in; }
        ...
        bool hit(const ray& r, float tmin, float tmax) const {
            for (int a = 0; a < 3; a++) {  // loop through axes
                ...
                if (tmax <= tmin) return false; // Ray missed objs's BB
            }
            return true; // hit!!
        }
...
bool hitable_list::bounding_box(aabb& box) const {
    ...
    aabb temp_box;
    if (!list[0]->bounding_box(temp_box)) return false;
    box = temp_box;
